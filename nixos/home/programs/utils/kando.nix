...
in {
  home.activation.kandoConfigInit = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    mkdir -p "$HOME/.config/kando"

	echo '${kandoConfig}' > "$HOME/.config/kando/config.json"
     
    echo '${kandoMenus}' > "$HOME/.config/kando/menus.json"
  '';
}
